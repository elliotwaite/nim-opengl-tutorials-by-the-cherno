...
proc main =
  ...
  block:
    ...
    var translationA = vec3(200.0f, 200.0, 0.0)
    var translationB = vec3(400.0f, 200.0, 0.0)
    ...
    while not window.windowShouldClose:
      ...
      block:
        let model = translate(mat4(1.0f), translationA)
      ...
      block:
        let model = translate(mat4(1.0f), translationB)
        var mvp = proj * view * model
        shader.`bind`
        shader.setUniformMat4f("u_MVP", mvp)
      ...
      igSliderFloat3("Translation A", translationA.arr, 0.0f, 960.0f)
      igSliderFloat3("Translation B", translationB.arr, 0.0f, 960.0f)
